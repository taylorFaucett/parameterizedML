...
def parameterized_training(iterations):
    ...
    ('neural network',
        Regressor(
            layers =[Layer("Sigmoid", units=3),Layer("Linear")],
            #learning_rate=0.01,
            n_iter=iterations,
            #learning_momentum=0.1,
            #batch_size=10,
...
def parameterized_output_plot():
    ...
    for idx, mu in enumerate(mu_values):
        ...
        inputs = data[:,0]
        outputs = data[:,1]
        plt.plot(inputs, outputs, alt_shape[idx], color=alt_color[idx], rasterized=True, markevery=75)
    
    line1, = plt.plot([-10,-10], label='Trained', marker='o', color='black', linestyle='None')
    line2, = plt.plot([-10,-10], label='Interpolated', marker='^', color='red', linestyle='None')    
