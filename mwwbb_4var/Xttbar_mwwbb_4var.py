...
def file_generate(root_file):
	...
	mjjj = root_export(root_file, 'xtt', 'mjjj')
	mlv = root_export(root_file, 'xtt', 'mlv')
	mjlv = root_export(root_file, 'xtt', 'mjlv')
...
def file_concatenater():
	...
	for signal, background in zip(sig_dat, bkg_dat):
		...
		np.savetxt('data/concatenated/ttbar_mx_%0.3f.dat' %sig[0,4], data_complete, fmt='%f')
...
def fixed_training():
	...
	for idx, jes in enumerate(jes_list):
		...
		training_data = input_data[:,0:4]
		jes_data = input_data[:,4]
		target_data = input_data[:,5]
		target_data1 = data1[:,5]
		jes = jes_data[0]
